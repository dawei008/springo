...
async def read_file_raw(artifact_id: str, path: str):
    """Stream a file's raw bytes — for binary content (PDFs, images) that
    can't ride the JSON ``read_file`` endpoint."""
    try:
        target = artifact_store._resolve_file_path(artifact_id, path)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))

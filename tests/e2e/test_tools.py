...
@pytest.mark.e2e
@pytest.mark.slow
async def test_tools_comparison_with_flask(
    flask_client: httpx.AsyncClient, 
    fastapi_client: httpx.AsyncClient
):
    """
    E2E Test: Compare tool execution results with Flask.
    对比 Flask 和 FastAPI 的工具执行结果。
    """
    try:
        # 测试 read_file
        test_file = "/tmp/comparison_test.txt"
        with open(test_file, 'w') as f:
            f.write("comparison test content")
        
        flask_resp = await flask_client.post("/v1/tools/execute", json={
            "name": "read_file",
            "input": {"path": test_file}
        })
        
        fastapi_resp = await fastapi_client.post("/v1/tools/execute", json={
            "name": "read_file",
            "input": {"path": test_file}
        })
        ...
        # 两者都应该成功
        if flask_data.get("success") and fastapi_data.get("success"):
            assert flask_data["result"].get("content") == fastapi_data["result"].get("content")
